...
static BIN_NAME: &str = "sha224sum";

fn sha1_digest(reader: &[u8]) -> String {
    let mut sha224hasher = Sha224Hasher::default();
    sha224hasher.write(reader);
...
fn output_hashes(cli: Cli) {
    ...
    for i in cli.name {
        let mut data = match File::open(&i) {
            Ok(dat) => dat,
            Err(e) => {
                if cli.strict {
        ...
        let digest = sha1_digest(buffer.as_ref());
...
fn check_hashes(cli: Cli) {
    ...
    for i in 0..hashes.len() - 1 {
        if &hashes[i] == &file_locs[i] {
            ...
            let mut file = File::open(&file_locs[i]).expect("could not open file");
            let mut buffer = vec![];
